  ...
  startGame(isBuildRun = false) {
    ...
    if (game.firstRun) {
      ...
      if (game.isSurvivalMode) {
        ...
        // level.levels.splice(0,  level.levels.length - 0 ["intro","skyscrapers", "rooftops", "warehouse", "highrise", "office", "aerie", "satellite", "sewers", "testChamber", "stronghold", "basement", "house"])
        // level.levels = ["testing"]; 
      } 
      else {
        if (game.isCommunityMaps) {
          level.levels.push("stronghold", "basement", "house", "newLevel");
        }
